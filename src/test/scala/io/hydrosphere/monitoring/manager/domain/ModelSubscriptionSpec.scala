    ...
    testM("should send all previous models to plugin") {
      ...
      val models = Set(
        Model(
          ...
          metadata = Map.empty,
          trainingDataPrefix = Some(uri"s3://bucket/"),
          inferenceDataPrefix = Some(uri"s3://bucket2/")
        ),
        Model(
          name = "b",
          version = 1,
          signature = Signature(Seq.empty, Seq.empty),
          metadata = Map.empty,
          trainingDataPrefix = Some(uri"s3://bucket3/"),
          inferenceDataPrefix = Some(uri"s3://bucket4/")
        )
      )
